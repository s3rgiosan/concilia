...
describe('matchTransactions', () => {
  describe('amount match (single candidate)', () => {
    ...
    it('rejects any cent difference for EUR receipts (no tolerance)', async () => {
      const { matchTransactions } = await load();
      const result = matchTransactions(
        [tx('tx-001', '2024-12-15', 'COMPRA', -5000)],
        [receipt('/r/a.pdf', 5001)],
      );
  ...
  describe('filename matching (pass 3)', () => {
    ...
    it('filename-matches EUR receipt when amount equals exactly', async () => {
      const { matchTransactions } = await load();
      // EUR receipt €10.00, transaction €10.00 — exact amount and name matches
      const r = { file: '/r/Spotify Premium Jan.pdf', amount_cents: 1000, confidence: 'high', provider_used: 'gemini', currency: 'EUR' };
      const result = matchTransactions(
        [tx('tx-001', '2024-12-15', 'SPOTIFY PREMIUM', -1000)],
        [r],
      );
      // Exact amount, so pass 1 (name+amount) catches it first
      assert.equal(result.transactions[0].status, 'MATCHED');
      assert.equal(result.transactions[0].notes, 'name_amount_match');
    });
  });
